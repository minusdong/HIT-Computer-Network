...
void printTips() {
    ...
    printf("|     -testgbn_Duplex [X] [Y] to test the gbn  |\n");
    printf("************************************************\n");
}
...
int main(int argc, char* argv[]) {
    ...
    while(true) {
        ...
        if (!strcmp(cmd, "-testgbn")) {
        ...
        else if(!strcmp(cmd, "-testgbn_Duplex")) {
			...
			// sendto(socketClient, "-testgbn_Duplex", strlen("-testgbn_Duplex")+1, 0, (SOCKADDR*)&addrServer, sizeof(SOCKADDR));
			...
			while(runFlag) {
				switch(stage) {
					...
					case 2://数据传输阶段
						...
						/*判断数据传输是否完成添加或修改*/
						if(seqIsAvailable() && totalSeq < totalPacket) {//totalSeq<=(totalPacket-1)：未传到最后一个数据包
						/*判断数据传输是否完成添加或修改*/
